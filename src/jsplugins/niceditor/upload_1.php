 ...
 * @version: 0.9.0
 */
 $mypath =  $_GET['apath'] == ""? "": "/".$_GET['apath'];

define('NICUPLOAD_PATH', '/opt/lampp/htdocs/thom/mvc/static/img/upload/artigos' . $mypath); // Set the path (relative or absolute) to
                                      // the directory to save image files

define('NICUPLOAD_URI', 'http://localhost/thom/mvc/static/img/upload/artigos' . $mypath);   // Set the URL (relative or absolute) to
...
if(!function_exists('json_encode')) {
    die('{"error" : "A fncão json_encode não existe neste servidor, tente instalar tal função ou trocar de servidor"}');
}
